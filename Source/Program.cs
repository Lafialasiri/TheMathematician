...
namespace TheMathematician
{
    static class Program
    {
        ...
        private class MainForm : Form
        {
            ...
            Label panel_Functions_PreviewLabel;
            Panel panel_Functions_ExpressionPanel;
            PictureBox panel_Functions_ExpressionPictureBox;
            ...
            public MainForm()
            {
                ...
                panel_Functions_input.TextChanged += (s, e) =>
                {
                    if (panel_Functions_input.Text == string.Empty)
                    {
                        panel_Functions_ExpressionPictureBox.Image = null;
                        return;
                    }

                    try
                    {
                        // Display the expression as an image using WpfMath
                        string expression = panel_Functions_input.Text;
                        var match1 = Regex.Match(expression, @"(.+?)\^\((.+?)\)(.*)");

                        // Validate the expression
                        if (Regex.IsMatch(expression, @"\\[a-zA-Z]+") || expression.Contains("_"))
                            return;

                        // Check for long exponents
                        if (match1.Success)
                        {
                            string baseVar = match1.Groups[1].Value;
                            string exponent = match1.Groups[2].Value;
                            string rest = match1.Groups[3].Value;

                            expression = $"{baseVar}^{{{exponent}}}{rest}";
                        }

                        // Initialize the WpfMath parser
                        var parser = WpfTeXFormulaParser.Instance;
                        var formula = parser.Parse(expression);
                        var environment = WpfTeXEnvironment.Create();

                        BitmapSource bitmapSource = formula.RenderToBitmap(environment);
                        ...
                        using (var ms = new MemoryStream())
                        {
                            ...
                            panel_Functions_ExpressionPictureBox.Image = new Bitmap(ms);
                        }
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine("Expression visualizing failed: " + ex.Message);
                        return;
                    }
                };
                ...
                panel_Functions_PreviewLabel = new Label()
                {
                ...
                panel_Functions_ExpressionPanel = new Panel()
                {
                    BackColor = Color.White,
                    Size = new Size(panel_Functions.Width - 19, 100),
                    Location = new Point(8, 88),
                    BorderStyle = BorderStyle.FixedSingle,
                    Anchor = AnchorStyles.Left | AnchorStyles.Top | AnchorStyles.Right,
                };
                panel_Functions_ExpressionPictureBox = new PictureBox()
                {
                    BorderStyle = BorderStyle.None,
                    Location = new Point(5, 5),
                    Size = new Size(panel_Functions_ExpressionPanel.Width - 10, panel_Functions_ExpressionPanel.Height - 10)
                };
                panel_Functions_ExpressionPanel.Controls.Add(panel_Functions_ExpressionPictureBox);
                ...
                panel_Functions_ZoomLimitCounter = new NumericUpDown()
                {
                    ...
                    Width = 50,
                    Anchor = AnchorStyles.Left | AnchorStyles.Top,
                    Location = new Point(162, 118 + 128)
                };
                panel_Functions_ZoomLimitLabel = new Label()
                {
                    ...
                    TextAlign = ContentAlignment.MiddleCenter,
                    AutoSize = true,
                    Location = new Point(112, 122 + 128)
                };
                ...
                */
                ...
                panel_Functions.Controls.Add(panel_Functions_PreviewLabel);
                panel_Functions.Controls.Add(panel_Functions_ExpressionPanel);
                ...
                graph = new Graph(
                    string.Empty,
                    (int)panel_Functions_xLimitCounter.Value,
                    (int)panel_Functions_yLimitCounter.Value,
                    float.Parse(panel_Functions_StepLimitTextBox.Text),
                    (int)panel_Functions_ZoomLimitCounter.Value,
                    graphSize,
                    graphPosition,
        ...
        private class AboutForm : Form
        {
            public AboutForm()
            {
                ...
                {
                    Text = $"- Version: {Program_Version}\n- Libraries:\n  ▻ {mXparser.VERSION_NAME} (github.com/mariuszgromada)\n  ▻ {Assembly.GetAssembly(typeof(TexFormula)).GetName().Version} {Assembly.GetAssembly(typeof(TexFormula)).GetName().Name} (github.com/ForNeVeR/xaml-math)\n\n\nDeveloped by {Program_DeveloperName}.",
